...
export function parseResponse(text: string): ParsedResponse {
  ...
  console.log('thoughtMatch', thoughtMatch);
  ...
  for (let i = 0; i < argsArray.length; i++) {
    ...
    // Improved string handling: Only strip quotes if the string is supposed to be enclosed in quotes
    ...
    if (expectedArg.type === 'number') {
      ...
      // No need to null-check after removing quotes since we're now directly assigning
